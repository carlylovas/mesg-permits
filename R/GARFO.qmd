...
write_csv(geo_codes, file = paste(here("Data/"), "geo_code_v1.csv"))
```
...
typos_v3 %>% 
  ...
  rename("PPORT" = "PPORT_CORRECTED") -> garfo_corrected  
...
typos_v3 %>% 
  ...
  rename("PPST" = "PPST_CORRECTED") %>%
  right_join(garfo_corrected) %>%
  arrange(AP_YEAR) -> garfo_corrected # geocode and combine with geo_codes (with nas removed)
