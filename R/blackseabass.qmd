...
biomass %>% 
  ...
  ggplot()+
  geom_col(aes(x = est_year, y = total_biomass, fill = season)) + #fill = "#07a3b7") +
...
#   group_by(STATE) %>%
#   nest() %>%
#   mutate(plot = map2(data, STATE, function(x,y){
#     ggplot(data = x) + 
#       geom_col(aes(x = YEAR, y = total_volume), fill = "#07a3b7", alpha = 0.7) +
#       xlim(c(1970,2021)) +
#       ggtitle(y) + ylab("Total landings (million lbs)") +
#       xlab("Year") + 
#       theme_gmri(strip.background = element_rect(fill = "transparent"),
#                 strip.text = element_text(color = "black"))
# })) -> landings_plots

 # gridExtra::marrangeGrob(landings_plots$plot, layout_matrix = matrix(1:3, ncol = 3), top = NULL)
...
# arrange north to south, if you can 
fed_license %>% 
  ...
  facet_wrap(~PPST, scales = "free_y", ncol = 3) +
