...
ggplot() +
  ...
  geom_point(data = recent, aes(x = avg_lon, y = avg_lat, color = season)) +
  scale_x_continuous(breaks = c(-70,-74)) +
  scale_y_continuous(breaks = c(38, 40, 42)) +
  facet_wrap(~est_year, ncol = 5) +
...
# use ports.geocoded.csv for this
...
# landings %>% 
#   select(PORT.NAME, STATE, lat, lon) %>% 
#   drop_na() %>% 
#   distinct() %>% 
#   mutate(avg_lat = round(lat, digits = 1)) %>% 
#   right_join(landings_conc) %>% 
#   drop_na() %>% 
#   arrange(YEAR) -> map
```
