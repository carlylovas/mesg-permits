...
licenses %>% 
  ...
  guides(alpha = guide_legend(title = "Total number issued")) +
  geom_vline(aes(xintercept = 1995), linetype = 1, color = "#07a3b7") +
...
stoll_time_series %>% 
  ...
  guides(alpha = guide_legend(title = "Total number issued")) +
  geom_vline(aes(xintercept = 2000), linetype = 1, color = "#07a3b7") +
...
# pre closure tallies
# pre_counts <- as.data.frame(pre_closure) %>% 
#   select("Northern Shrimp") %>% 
#   rowid_to_column() %>%
#   mutate(total = sum(`Northern Shrimp`)) %>% 
#   mutate(freq = (`Northern Shrimp`/total)*100)
