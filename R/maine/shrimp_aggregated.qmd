...
participation %>%
  filter(participation == "true" & n_distinct(`Northern Shrimp`) >= 2) %>%
  ungroup() %>%
...
portfolio %>% 
  ...
  ggtitle("Shrimper portfolios", subtitle = "Includes the 42 shrimpers who left all state fisheries in years leading up to the Northern shrimp closure") + ylab("Total licenses issued") + xlab("Year") +
...
#        filter(landings_number %in% shrimpers$landings_number) %>% 
#        group_by(license_year) %>% 
#        mutate(n_harvesters = n_distinct(landings_number)) %>%
#        group_by(license_year, n_harvesters) %>% 
#        summarise(across(c(`Scallop (Dragger)`:`Menhaden`), sum)))
...
# NOTE TO SELF: use the co-matrix?
# pre_closure <- as.data.frame(shrimp_matrix$co_matrix[[1]]) %>% 
#   select("Northern Shrimp")
...
portfolio %>%
  ...
  group_by(closure) %>%
  summarise(across(everything(), sum)) %>%
  pivot_longer(cols = 2:27, names_to = "license_type", values_to = "count") %>%
  filter(!license_type == "Northern Shrimp") %>%
  group_by(closure) %>%
  mutate(total = sum(count),
         freq  = (count/total)*100) %>%
  select(license_type, closure, freq) %>%
  pivot_wider(names_from = "closure", values_from = "freq") %>%
  mutate(increase = ifelse(`Pre-closure` < `Post-closure`, T, F)) -> all_proportions
