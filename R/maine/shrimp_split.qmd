...
portfolio %>% 
  filter(landings_number %in% shrimpers$landings_number & license_year >= 2000) -> shrimp_portfolios
...
shrimp_matrix <- shrimp_portfolios %>%
  arrange(license_year) %>%
  mutate(closure = ifelse(license_year >= 2014, "post-closure", "pre-closure")) %>%
...
shrimp_portfolios %>% 
  ...
  scale_fill_gmri()+
  theme_gmri(legend.position = "bottom") # try species level -> switch from shrimp to what (if to anything)
...
shrimp_portfolios %>% 
  ...
  mutate(closure = ifelse(license_year <= 2014, "pre-closure", "post-closure")) -> shrimper_counts
...
portfolio %>% 
  ...
  distinct() %>% 
  mutate(closure = ifelse(license_year <= 2014, "pre_closure", "post_closure")) %>% 
...
shrimp_portfolios %>% 
  ...
  guides(fill = guide_legend(ncol = 4)) +
  # scale_fill_gmri()+
  theme_gmri(legend.position = "bottom") # their pre-closure portfolios weren't that different from those who stayed
...
pre_closure %>%
  ...
  # facet_wrap(~factor(participation, levels = c("pre-closure", "post-closure")), scales = "free_x", ncol = 2) +
  guides(fill = guide_legend(ncol = 4)) +
  # scale_fill_gmri()+
  theme_gmri(legend.position = "bottom") # proportions next? 
