...
ind_lic_portfolio %>%
  ...
  select(license_year, landings_number, licensed, divisions, row) %>%
  pivot_wider(names_from=divisions,values_from=licensed,names_expand=TRUE,values_fill = list(licensed=0)) %>%
  select(-row)%>%
...
species_portfolio %>%
  ...
  pivot_longer(cols = 2:27, names_to = "divisions", values_to = "count") %>%
  filter(!count == 0) %>% 
  select(license_year, divisions) %>% 
  distinct() %>%
...
species_portfolio %>%
  ...
  select(!landings_number) %>%
  pivot_longer(cols = 2:27, names_to = "divisions", values_to = "count") %>%
  group_by(license_year, divisions) %>%
  summarise(total = sum(count)) %>%
  group_by(divisions) %>%
  nest() %>%
  left_join(license_divisions %>% select(divisions, license_group) %>% distinct())-> license_time_series
...
annual_co_matrix <- species_portfolio %>%
  ...
  mutate(co_matrix = map2(data, license_groups, function(x, y){
    df <- x %>% select(y$divisions) # this filters the portfolio to feature only species harvested in that year. If you want to run with all species, command out this line
...
ggplot(annual_metrics) +
  ...
  theme_gmri(axis.line.y  = element_line(color = "black")) +
  ylim(c(0.5,1))
