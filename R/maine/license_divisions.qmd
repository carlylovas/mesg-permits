...
Groups, or *modules*, are denoted by color. Community structure is determined by a multi-level optimization of modularity, known as Louvain clustering.
...
annual_graphs <- annual_metrics %>%
  mutate(ggraph = map2(graph_obj, license_year, function(x, y){
    ...
    out <- ggraph(x, 'kk') + 
      geom_node_point(aes(color = community), size = 7, alpha = 0.8, show.legend = FALSE) +
      geom_edge_link(aes(width = weight), alpha = 0.6, show.legend = FALSE) + 
      coord_fixed() +
