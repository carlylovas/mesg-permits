...
garfo_clean %>%
  ...
  filter(PPST %in% c("ME", "NH", "MA", "RI", "CT", "NY", "NJ", "DE", "MD", "VA", "NC") & AP_YEAR >= 1997) %>%
  filter(!is.na(COUNT)) %>%
  filter(!COUNT == 0) %>%
...
garfo_holdings$category[garfo_holdings$PERMIT_TYPE %in% c("SQUID_MACK_BUTTER_1", "SQUID_MACK_BUTTER_1A", "SQUID_MACK_BUTTER_1B", "SQUID_MACK_BUTTER_1C")] = "Mackerel" # parsing out mackerel only permits
...
garfo_holdings %>%
  group_by(PPST, AP_YEAR, category) %>% # sub category
  summarise(count = sum(COUNT)) -> total 

# by state
total %>% 
  group_by(PPST, category) %>% # sub category
...
bsb %>% 
  group_by(PPST) %>%
  filter(z_score_manual %in% max(z_score_manual))
...
ggplot(data = bsb) +
  ...
  ylim(c(-2,2)) +
  ggtitle("Standardized scores", subtitle = "Z-scores") + 
  xlab("Year") + ylab("Distance from mean") +
  theme_gmri() -> plot1
  # max licence counts
...
ggsave(here("Outputs", "zscore_bsb.png"), plot1, width = 7, height = 5, units = "in", bg = "white")


mack <- z_score %>% 
  filter(category == "Mackerel" & PPST %in% c("NY", "RI", "NH", "ME"))
...
ggsave(here("Outputs", "zscore_mack.png"), plot2, width = 7, height = 5, units = "in", bg = "white")
