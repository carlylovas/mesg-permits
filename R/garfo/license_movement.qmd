...
# Static plot
weighted_lat %>% 
  filter(license %in% c("SQUID_MACK_BUTTER_1", "SQUID_MACK_BUTTER_1A", "SQUID_MACK_BUTTER_1B", "SQUID_MACK_BUTTER_1C")) %>%
  mutate(comname = "longfin squid") %>%
  left_join(weighted_data %>% filter(comname == "longfin squid" & est_year >= 1996)) %>%
  ggplot() + 
  ...
  scale_color_manual(labels = c("Biomass", "Category 1", "1A", "1B", "1C"), values = c("#00608a", "#ea4f12", "#057872", "#ebcb27", "#38431d")) + 
...
# Static plot
weighted_lat %>% 
  filter(license == "SQUID_MACK_BUTTER_5") %>%
  mutate(comname = "northern shortfin squid") %>%
  left_join(weighted_data %>% filter(comname == "northern shortfin squid" & est_year >= 1996)) %>%
  ggplot() + 
  geom_line(aes(x = est_year, y = avg_lat, color = comname), linetype = 2, linewidth = 1) +
  geom_line(aes(x = AP_YEAR, y = lat_weighted, color = license), linewidth = 1) +
  scale_color_manual(labels = c("Biomass", "License"), values = c("#00608a", "#ea4f12")) + 
  xlab("Year") + ylab("Latitude") + ggtitle("Center of latitude", subtitle = "1996-2023") +
  # scale_y_continuous(limits = c(34,46), breaks = c(34, 40, 46)) +  
  guides(color = guide_legend(override.aes = list(linewidth = 2))) +
  theme_gmri(legend.title = element_blank(),
...
weighted_lat %>% 
  ...
  geom_line(aes(x = est_year, y = avg_lat, color = comname), linetype = 2, linewidth = 1) +
  geom_line(aes(x = AP_YEAR, y = lat_weighted, color = license), linewidth = 1) +
  scale_color_manual(labels = c("Biomass", "Category 4", "Tier 1", "Tier 2", "Tier 3"), values = c("#00608a", "#ea4f12", "#057872", "#ebcb27", "#38431d")) + 
  xlab("Year") + ylab("Latitude") + ggtitle("Center of latitude", subtitle = "1996-2023") +
  # scale_y_continuous(limits = c(34,46), breaks = c(34, 40, 46)) +  
  guides(color = guide_legend(override.aes = list(linewidth = 2))) +
  theme_gmri(legend.title = element_blank(),
