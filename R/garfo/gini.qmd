...
garfo_geo %>% 
  ...
  mutate(total_count = sum(count),
         freq = (count/total_count)*100) -> license_freq


license_freq %>% 
  group_by(AP_YEAR, PPST) %>% #
  summarise(GINI = Gini(count)) -> gini_state # I think this way yield inequality across license types, not states 

ggplot(gini_state) +
  geom_line(aes(x = AP_YEAR, y = GINI, color = PPST)) +
  theme_gmri() +
  scale_color_gmri()
