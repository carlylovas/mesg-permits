...
portfolio <- read_rds(here("R", "maine","stoll_portfolio.rds")) %>% 
...
portfolio %>% 
  ...
  mutate(participation = ifelse(`Northern Shrimp` == 0, "false", "true")) -> shrimp_fishery_participation 
...
portfolio %>% 
  filter(landings_number %in% shrimpers$landings_number)-> shrimp_portfolios
...
shrimp_portfolios %>% 
  ...
  group_by(license_year, closure, license_type) %>%
  summarise(count = sum(count)) %>%
  # filter(closure == "pre-closure") %>%
  ggplot() +
  geom_col(aes(x = license_year, y = count, fill = license_type), color = "white") +
...
shrimp_portfolios %>% 
  ...
  relocate("pre-closure", .before = "post-closure") %>% 
  filter(is.na(`post-closure`)) -> ex_shrimpers

# all harvesters before and after shrimp
portfolio %>% 
  select(license_year, landings_number) %>% 
  distinct() %>% 
  mutate(closure = ifelse(license_year <= 2013, "pre_closure", "post_closure")) %>% 
...
shrimp_portfolios %>% 
  pivot_longer(cols = 3:28, names_to = "license_type", values_to = "count") %>%
  filter(!count == 0) %>%
  mutate(closure = ifelse(license_year <= 2013, "pre-closure", "post-closure")) %>% 
...
participants %>% 
  ...
  pivot_longer(3:28, names_to = "license", values_to = "count") %>% 
  filter(!count == 0) %>% 
  mutate(closure = ifelse(license_year <= 2013, "pre-closure", "post-closure")) %>%
  group_by(license_year, license, closure) %>%
  summarise(count = sum(count)) %>%
  group_by(license, closure) %>%
  summarise(avg_count = mean(count)) -> averages # this wont work for an adjacency matrix/chord diagram

# average yearly proportion, maybe? 
