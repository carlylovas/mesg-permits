...
license_types %>%
  filter(!target_species %in% c("Demo","Post-Harvest", "Non-resident", "Recreational")) %>%
  ...
  # full_join(license_divisions) %>% 
  drop_na() -> license_types # the one NA here is sut 

license_types %>% 
  gt() %>% 
  cols_hide(c(license_type)) %>%
  cols_label(
    target_species   = md("**Target Species**"),
    description      = md("**License Type**"))
    # group            = md("**Stoll 2016 Grouping**"))
```
...
ind_lic_portfolio %>%
  ...
  select(license_year, landings_number, licensed, target_species, row) %>%
  pivot_wider(names_from=target_species,values_from=licensed,names_expand=TRUE,values_fill = list(licensed=0)) %>%
  select(-row)%>%
...
species_portfolio %>% 
  pivot_longer(cols = 3:21, names_to = "target_species", values_to = "count") %>%
  filter(!count == 0) %>%
  ungroup(landings_number) %>%
  # left_join(license_divisions) %>%
  select(license_year, target_species) %>%
  distinct() %>%
...
species_portfolio %>% 
  ...
  # pivot_longer(cols = 3:21, names_to = "group", values_to = "count") %>%
  filter(!count == 0) %>%
  ungroup(landings_number) %>%
  # left_join(license_divisions) %>%
  # select(license_year, target_species, count) %>%
  # distinct() %>%
  group_by(license_year, target_species) %>%
...
ggplot(data = species_proportions, aes(x = license_year, y = proportion, fill = target_species)) +
